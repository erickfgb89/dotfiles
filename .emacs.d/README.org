...
*** flymake
    #+BEGIN_SRC emacs-lisp
      (use-package flycheck
        ...
        :config (setq-default flycheck-disabled-checkers '(chef-foodcritic)))
         ...
         (if (projectile-project-p)
             (let* ((project-name (projectile-project-name))
               ...
               (if workgroup ;wg exists
                 ...
                 (other-window 1)
                 (select-or-start-eshell)
                 (other-window -1)))))

       ;; (defadvice other-window (before ignore-small-eshell-windows activate)
       ;;   "Skip eshell windows in a small lower window during interactive other-window"
       ;;   (if (and
       ;;        (= 1 (ad-get-arg 0))
       ;;        (string-match-p ".*eshell.*"
